...
public class InformeRepository : IInformeRepository
{
    ...
    (
        SELECT
            ...
            (
                ...
                ORDER BY COALESCE(c.fecha_confirmacion, c.fecha_validacion) DESC, c.id_carga DESC
            ) AS rn
        FROM carga c
        WHERE c.activo = 1
    ...
    (
        ...
        (
            ...
            ORDER BY COALESCE(c2.fecha_confirmacion, c2.fecha_validacion) DESC, c2.id_carga DESC
        ) conf
        WHERE v.rn = 1
    )
    SELECT
    ...
    FROM envios e
    INNER JOIN usuario ucarga ON ucarga.id_usuario = e.id_usuario_carga
    INNER JOIN catalogo_entidad_federativa ef ON ef.id_entidad_federativa = e.id_entidad_federativa
    ORDER BY ef.nombre, e.anio_corte DESC, e.mes_corte DESC
    OPTION (RECOMPILE);
";
    ...
    private async Task<List<IDictionary<string, object?>>> QueryDictionaryAnioAsync(string sql, int anioCorte, int? idEntidadFederativa = null, string modoPlano = "CONFIRMADO", int mesUltimoCorte = 0)
    {
        using var connection = _dbConnectionFactory.CrearConexion();
    ...
    public async Task<List<IDictionary<string, object?>>> ObtenerSabanaEstatalDelitosAsync(int anioCorte, int? idEntidadFederativa, string modoPlano, int mesUltimoCorte)
    {
        var sql = @"
    ...
    public async Task<List<IDictionary<string, object?>>> ObtenerSabanaEstatalVictimasAsync(int anioCorte, int? idEntidadFederativa, string modoPlano, int mesUltimoCorte)
    {
        var sql = @"
                    WITH pendientes_rankeadas AS (
                SELECT
