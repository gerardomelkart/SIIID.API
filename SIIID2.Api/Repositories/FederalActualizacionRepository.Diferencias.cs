...
public partial class FederalActualizacionRepository
{
    ...
    public async Task<ActualizacionDiferenciasResponse?> ObtenerDiferenciasAsync(string codigoReferencia, int idUsuarioConsulta, int limitePorSeccion)
    {
        ...
        using (var connection = _dbConnectionFactory.CrearConexion())
            contexto = await ObtenerContextoActualizacionAsync(connection, codigoReferencia, idUsuarioConsulta);
    ...
    private static async Task<ActualizacionDiferenciasContexto?> ObtenerContextoActualizacionAsync(IDbConnection connection, string codigoReferencia, int idUsuarioConsulta)
    {
        var sql = @"
        SELECT TOP 1
            id_federal_carga AS IdCarga,
            codigo_referencia AS CodigoReferencia,
            c.mes_corte AS MesCorte,
            c.anio_corte AS AnioCorte
        FROM dbo.federal_carga c
        INNER JOIN dbo.usuario u ON u.id_usuario = @IdUsuarioConsulta AND u.activo = 1
        ...
        WHERE c.codigo_referencia = @CodigoReferencia
          ...
          AND c.activo = 1
          AND (r.rol = N'SUPER_USUARIO' OR c.id_usuario_carga = @IdUsuarioConsulta);
        ";
        ...
        var sql = @"
            ;WITH carpetas_actuales AS (
                SELECT
                    ...
                    ci.fecha_inicio,
                    ci.resumen_hechos
                FROM federal_carpeta_investigacion ci
                INNER JOIN federal_carga c
                    ON c.id_federal_carga = ci.id_federal_carga
                WHERE c.mes_corte = @MesCorte
            ...
            SELECT
                ...
                c.rmen_de_hchos
            FROM federal_carga_tmp_carpeta c
            WHERE c.id_federal_carga = @IdCarga
              AND c.activo = 1
        )
    ...
    private static async Task<List<ActualizacionDiferenciaRow>> ObtenerDiferenciasDelitosAsync(IDbConnectionFactory dbConnectionFactory, ActualizacionDiferenciasContexto contexto, int limitePorSeccion)
    {
        ...
        ;WITH delitos_actuales AS (
            ...
            FROM federal_delito d
            INNER JOIN federal_carpeta_investigacion ci
            ...
            LEFT JOIN federal_catalogo_modalidad_delito md
        ...
        delitos_tmp AS (
            SELECT
                ...
                d.nom_col_hchos,
                cp.id_codigo_postal,
                TRY_CONVERT(decimal(10,6), NULLIF(REPLACE(d.coord_x, ',', '.'), '')) AS coordenada_x,
                TRY_CONVERT(decimal(10,6), NULLIF(REPLACE(d.coord_y, ',', '.'), '')) AS coordenada_y,
                d.dom_hchos
            FROM federal_carga_tmp_delito d
            INNER JOIN federal_catalogo_modalidad_delito md
            ...
            WHERE d.id_federal_carga = @IdCarga
              AND d.activo = 1
        )
        SELECT TOP (@LimiteFilas)
            'delitos' AS Seccion,
        ...
        CROSS APPLY (
            VALUES
                ...
                ('cp', CONVERT(varchar(max), da.codigo_postal_fiscalia), CONVERT(varchar(max), dt.cp_excel), CONVERT(varchar(max), da.codigo_postal_fiscalia), CONVERT(varchar(max), NULLIF(LTRIM(RTRIM(dt.cp_excel)), ''))),
    ...
    ";
        ...
        /*
            Un federal_delito genera hasta veinte filas de campos.
        */
    ...
    private static async Task<List<ActualizacionDiferenciaRow>> ObtenerDiferenciasVictimasAsync(IDbConnectionFactory dbConnectionFactory, ActualizacionDiferenciasContexto contexto, int limitePorSeccion)
    {
        ...
        ;WITH victimas_actuales AS (
            ...
            FROM federal_victima v
            INNER JOIN federal_delito d
        ...
        victimas_tmp AS (
            ...
            LEFT JOIN catalogo_presenta_discapacidad disc
               ...
               AND disc.activo = 1
            WHERE v.id_federal_carga = @IdCarga
              AND v.activo = 1
        )
        SELECT TOP (@LimiteFilas)
            'victimas' AS Seccion,
