...
public partial class FederalActualizacionRepository
{
    ...
    private static async Task AplicarActualizacionAsync(SqlConnection connection, SqlTransaction transaction, long idFederalCarga, int idUsuarioRegistro, int idUsuarioConfirmacion)
    {
        ...
        SELECT v.id_ci, v.id_delito, v.id_vicf, tv.id_tipo_victima, tvm.id_tipo_victima_moral, sx.id_sexo, gen.id_genero, nac.id_nacionalidad, pob.id_pertenece_poblacion_indigena, disc.id_presenta_discapacidad,
               COALESCE(TRY_CONVERT(date, NULLIF(v.fha_nac, N''), 103), TRY_CONVERT(date, NULLIF(v.fha_nac, N''))) AS fecha_nacimiento, TRY_CONVERT(smallint, NULLIF(v.edad, N'')) AS edad, NULLIF(v.nombre_vicfem, N'') AS nombre_vicfem, NULLIF(v.primer_apellido_vicfem, N'') AS primer_apellido_vicfem, NULLIF(v.segundo_apellido_vicfem, N'') AS segundo_apellido_vicfem, NULLIF(v.curp_vicfem, N'') AS curp_vicfem
        INTO #victimas_tmp
        ...
        INSERT INTO dbo.federal_victima_historico (id_federal_victima, id_federal_delito, identificador_victima_fiscalia, id_tipo_victima, id_tipo_victima_moral, id_sexo, id_genero, id_nacionalidad, id_pertenece_poblacion_indigena, id_presenta_discapacidad, fecha_nacimiento, edad, nombre_vicfem, primer_apellido_vicfem, segundo_apellido_vicfem, curp_vicfem, id_usuario_registro, fecha_registro, id_federal_carga, id_usuario_modificacion, id_federal_carga_nueva, tipo_movimiento, fecha_modificacion, activo)
        SELECT v.id_federal_victima, v.id_federal_delito, v.identificador_victima_fiscalia, v.id_tipo_victima, v.id_tipo_victima_moral, v.id_sexo, v.id_genero, v.id_nacionalidad, v.id_pertenece_poblacion_indigena, v.id_presenta_discapacidad, v.fecha_nacimiento, v.edad, v.nombre_vicfem, v.primer_apellido_vicfem, v.segundo_apellido_vicfem, v.curp_vicfem, v.id_usuario_registro, v.fecha_registro, v.id_federal_carga, @IdUsuarioConfirmacion, @IdFederalCarga,
        ...
        UPDATE v SET v.id_tipo_victima = vt.id_tipo_victima, v.id_tipo_victima_moral = vt.id_tipo_victima_moral, v.id_sexo = vt.id_sexo, v.id_genero = vt.id_genero, v.id_nacionalidad = vt.id_nacionalidad, v.id_pertenece_poblacion_indigena = vt.id_pertenece_poblacion_indigena, v.id_presenta_discapacidad = vt.id_presenta_discapacidad, v.fecha_nacimiento = vt.fecha_nacimiento, v.edad = vt.edad, v.nombre_vicfem = vt.nombre_vicfem, v.primer_apellido_vicfem = vt.primer_apellido_vicfem, v.segundo_apellido_vicfem = vt.segundo_apellido_vicfem, v.curp_vicfem = vt.curp_vicfem, v.id_federal_carga = @IdFederalCarga
        ...
        WHERE NOT EXISTS (SELECT 1 FROM dbo.federal_carpeta_investigacion ci INNER JOIN dbo.federal_carga c ON c.id_federal_carga = ci.id_federal_carga WHERE ci.identificador_carpeta_fiscalia = ct.id_ci AND ci.activo = 1 AND c.activo = 1 AND (c.id_federal_carga = @IdFederalCarga OR (c.mes_corte = @MesCorte AND c.anio_corte = @AnioCorte AND c.estado IN (N'CONFIRMADO', N'CONFIRMADO_ACTUALIZACION'))));
        ...
        INSERT INTO dbo.federal_victima (id_federal_delito, identificador_victima_fiscalia, id_tipo_victima, id_tipo_victima_moral, id_sexo, id_genero, id_nacionalidad, id_pertenece_poblacion_indigena, id_presenta_discapacidad, fecha_nacimiento, edad, nombre_vicfem, primer_apellido_vicfem, segundo_apellido_vicfem, curp_vicfem, id_usuario_registro, fecha_registro, id_federal_carga, activo)
        SELECT d.id_federal_delito, vt.id_vicf, vt.id_tipo_victima, vt.id_tipo_victima_moral, vt.id_sexo, vt.id_genero, vt.id_nacionalidad, vt.id_pertenece_poblacion_indigena, vt.id_presenta_discapacidad, vt.fecha_nacimiento, vt.edad, vt.nombre_vicfem, vt.primer_apellido_vicfem, vt.segundo_apellido_vicfem, vt.curp_vicfem, @IdUsuarioRegistro, SYSDATETIME(), @IdFederalCarga, 1
