...
public static class CodigoPostalCatalogoHelper
{
    ...
    public static async Task<bool> EsSuperUsuarioAsync(IDbConnectionFactory dbConnectionFactory, int idUsuario)
    {
        const string sql = @"
    ...
    public static async Task<Resultado?> AsegurarAsync(IDbConnectionFactory dbConnectionFactory, string codigoReferencia, bool remapearFinal = false, bool permitirConfirmada = false)
    {
        using var connection = (SqlConnection)dbConnectionFactory.CrearConexion();
        await connection.OpenAsync();
        using var transaction = (SqlTransaction)await connection.BeginTransactionAsync(IsolationLevel.Serializable);

        try
        {
            var idCarga = await ObtenerIdCargaAsync(connection, transaction, codigoReferencia, permitirConfirmada);
            ...
            var sinPlantilla = await ContarCodigosSinPlantillaAsync(connection, transaction, idCarga.Value);
            ...
            return new Resultado
            {
                ...
                CodigosReactivados = reactivados,
                CodigosInsertados = insertados,
                CodigosSinPlantilla = sinPlantilla,
                DelitosRemapeados = remapeados
            };
        }
        catch
        {
    ...
    private static async Task<long?> ObtenerIdCargaAsync(SqlConnection connection, SqlTransaction transaction, string codigoReferencia, bool permitirConfirmada)
    {
        const string sql = @"
            ...
            WHERE c.codigo_referencia = @CodigoReferencia
              ...
              AND (
                    ...
                    OR (
                        @PermitirConfirmada = 1
                        AND c.estado IN (N'CONFIRMADO', N'CONFIRMADO_ACTUALIZACION')
                    )
                  )
            ORDER BY c.id_carga DESC;";

        return await connection.QueryFirstOrDefaultAsync<long?>(sql, new { CodigoReferencia = codigoReferencia, PermitirConfirmada = permitirConfirmada }, transaction);
    }

    private static string SqlCodigosCarga => @"
        SELECT DISTINCT
    ...
    private static async Task<int> ContarCodigosSinPlantillaAsync(SqlConnection connection, SqlTransaction transaction, long idCarga)
    {
        var sql = $@"
            ;WITH codigos AS (
                {SqlCodigosCarga}
            ...
            WHERE NOT EXISTS (
                SELECT 1
                FROM dbo.catalogo_codigo_postal plantilla
                WHERE plantilla.id_municipio = f.id_municipio
            );";

        return await connection.ExecuteScalarAsync<int>(sql, new { IdCarga = idCarga }, transaction);
    }
